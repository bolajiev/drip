  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-6xl flex-col px-5 sm:px-8">
      <header className="flex items-center justify-between border-b border-rule py-4">
        <button onClick={() => setView("customer")} className="flex items-center gap-2.5">
          ...
          <span className="mt-px hidden font-mono text-[10px] text-ink-soft sm:block">XRP SUBSCRIPTION STREAMING</span>
        </button>
      ...
      <section className="border-b border-rule py-10 sm:py-14">
        <h1 className="max-w-3xl font-sans text-4xl font-bold leading-[1.05] tracking-tight sm:text-6xl">
          Pay once in XRP.
          <br />
          It <span className="underline decoration-acid-deep decoration-4 underline-offset-4">streams</span> to the
          merchant.
        </h1>
        <p className="mt-4 max-w-xl font-mono text-xs leading-relaxed text-ink-soft sm:text-sm">
          One XRPL payment mints FXRP into a cancelable stream. The merchant draws down what&apos;s accrued;
          cancel anytime and the unstreamed remainder returns to you. No auto-debits, no chargebacks.
        </p>
        <div className="mt-6">
          <NetworkNote />
        </div>
        ...
        {!isConnected ? (
          <div className="border border-dashed border-rule px-6 py-16 text-center">
            ...
            <p className="mt-2 font-mono text-[11px] text-ink-soft">
              MetaMask / Rabby · chain id 114 · RPC {coston2.rpcUrls.default.http[0]}
            </p>
          </div>
        ) : view === "merchant" ? (
      ...
      <footer className="border-t border-rule py-6 font-mono text-[10px] leading-relaxed text-ink-soft">
        ...
        <p className="mt-3 text-[10px]">COSTON2 TESTNET — HACKATHON DEMO. NOT FINANCIAL ADVICE.</p>
      </footer>
    </div>
  );
}
